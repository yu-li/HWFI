        ...
        class Decoder(torch.nn.Module):
            def __init__(self, intLevel):
                ...
                self.netThr = torch.nn.Sequential(
                    torch.nn.Conv2d(in_channels=intCurrent + 128 + 128,
                                    ...
                                    kernel_size=3,
                                    stride=1,
                                    padding=1), torch.nn.LeakyReLU(inplace=False, negative_slope=0.1))
            ...
            def backwarp(self, tenInput, tenFlow):
                if str(tenFlow.size()) not in self.backwarp_tenGrid:
                    ...
                    tenVertical = torch.linspace(-1.0, 1.0,
                                                 tenFlow.shape[2]).view(1, 1, tenFlow.shape[2],
                                                                        1).expand(tenFlow.shape[0], -1, -1,
                                                                                  tenFlow.shape[3])

                    self.backwarp_tenGrid[str(tenFlow.size())] = torch.cat([tenHorizontal, tenVertical], 1).cuda()

                if str(tenFlow.size()) not in self.backwarp_tenPartial:
                    self.backwarp_tenPartial[str(tenFlow.size())] = tenFlow.new_ones(
                ...
                tenInput = torch.cat([tenInput, self.backwarp_tenPartial[str(tenFlow.size())]], 1)

                tenOutput = torch.nn.functional.grid_sample(
                    input=tenInput,
                    grid=(self.backwarp_tenGrid[str(tenFlow.size())] + tenFlow).permute(0, 2, 3, 1),
                    mode='bilinear',
                    padding_mode='zeros',
                )
            ...
            def norm_before_costvolume(self, tenList):
                ele = torch.cat(tenList, dim=1)
                n, c, h, w = ele.shape
                mean = ele.view(n, -1).mean(dim=1, keepdim=True).view(n, 1, 1, 1)
                var = ele.view(n, -1).var(dim=1, keepdim=True).view(n, 1, 1, 1)
                std = torch.sqrt(var + 1e-16)
                res = [(f - mean) / std for f in tenList]
                return res
            ...
            def forward(self, tenFirst, tenSecond, objPrevious):
                ...
                if objPrevious is None:
                    ...
                    tenFeat = None
                    ###
                    # NtenFirst, NtenSecond = self.norm_before_costvolume([tenFirst, tenSecond])
                    ###
                    ...
                    tenVolume = torch.nn.functional.leaky_relu(input=correlation.FunctionCorrelation(
                        tenFirst=tenFirst,
                        tenSecond=self.backwarp(tenInput=tenSecond, tenFlow=tenFlow * self.fltBackwarp)),
                                                               negative_slope=0.1,
                                                               inplace=False)
            ...
            def __init__(self):
                ...
                self.netMain = torch.nn.Sequential(
                    torch.nn.Conv2d(in_channels=81 + 32 + 2 + 2 + 128 + 128 + 96 + 64 + 32,
                                    ...
                                    stride=1,
                                    padding=1,
                                    dilation=1), torch.nn.LeakyReLU(inplace=False, negative_slope=0.1),
